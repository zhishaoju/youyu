...
public class SplashTencentActivity extends BaseActivity implements SplashADListener,
  ...
  /**
   * 为防止无广告时造成视觉上类似于"闪退"的情况，设定无广告时页面跳转根据需要延迟一定时间，demo 给出的延时逻辑是从拉取广告开始算开屏最少持续多久，仅供参考，开发者可自定义延时逻辑，如果开发者采用demo
  ...
  private String getPosId() {
    ...
    return TextUtils.isEmpty(posId) ? Contants.AD_TENCENT_DISPLAY : posId;
  }
   ...
   *
   * Demo代码里是一个基本的权限申请示例，请开发者根据自己的场景合理地编写这部分代码来实现权限申请。 注意：下面的`checkSelfPermission`和`requestPermissions`方法都是在Android6.0的SDK中增加的API，如果您的App还没有适配到Android6.0以上，则不需要调用这些方法，直接调用广点通SDK即可。
   */
  ...
  /**
   * 倒计时回调，返回广告还将被展示的剩余时间。 通过这个接口，开发者可以自行决定是否显示倒计时提示，或者还剩几秒的时候显示倒计时
   *
  ...
  public void onNoAD(AdError error) {
    final String str = String
        .format("LoadSplashADFail, eCode=%d, errorMsg=%s", error.getErrorCode(),
            error.getErrorMsg());
    Log.i("AD_DEMO", str);
    ...
    handler.postDelayed(new Runnable() {
      ...
      public void run() {
        if (needStartDemoList) {
          SplashTencentActivity.this
              .startActivity(new Intent(SplashTencentActivity.this, DemoListActivity.class));
        }
        SplashTencentActivity.this.finish();
      }
    }, shouldDelayMills);
  }
