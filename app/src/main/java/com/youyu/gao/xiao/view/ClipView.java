...
//                    this.getWidth() - mHorizontalPadding, this.getHeight() / 2 + clipWidth / 2, borderPaint);
      ...
      //绘制中间的矩形
      canvas.drawRect(mHorizontalPadding, Utils.dip2px(60),
          this.getWidth() - mHorizontalPadding,
          getHeight() - Utils.dip2px(60), paint);
      //绘制白色的矩形边框
      canvas.drawRect(mHorizontalPadding, Utils.dip2px(60),
          this.getWidth() - mHorizontalPadding,
          getHeight() - Utils.dip2px(60), borderPaint);

      LogUtil.showDLog(TAG,
          "this.getHeight() / 2 - clipWidth / 2 - clipWidth / 10 = " + (this.getHeight() / 2
              - clipWidth / 2 - clipWidth / 10));
      LogUtil.showDLog(TAG,
          "this.getHeight() / 2 + clipWidth / 2 + clipWidth / 10 = " + (this.getHeight() / 2
              + clipWidth / 2 + clipWidth / 10));

      LogUtil.showDLog(TAG, "Utils.dip2px(60) = " + Utils.dip2px(60));

      LogUtil.showDLog(TAG, "getHeight() - Utils.dip2px(60) = " + (getHeight() - Utils.dip2px(60)));

      int w = Utils.getWH()[0];
      int h = Utils.getWH()[1];

      LogUtil.showDLog(TAG, "w = " + w + "; h = " + h);
...
//    rect.top = (this.getHeight() / 2 - clipRadiusWidth);
    rect.top = Utils.dip2px(60);
    //高度的一半 + 圆的半径
//    rect.bottom = (this.getHeight() / 2 + clipRadiusWidth);
    rect.bottom = this.getHeight() - Utils.dip2px(60);
    return rect;
  }
