...
import static com.youyu.gao.xiao.utils.Utils.getRealFilePathFromUri;
...
public class ClipViewLayout extends RelativeLayout {
  ...
  public void initSrcPic(Uri uri) {
    ...
    int w = Utils.getWH()[0];
    int h = Utils.getWH()[1];
    Bitmap bitmap = decodeSampledBitmap(path, w, h);
    if (bitmap == null) {
      return;
    }
  ...
  public Bitmap clip() {
    ...
    try {
      cropBitmap = Bitmap
          .createBitmap(imageView.getDrawingCache(), rect.left, rect.top, rect.width(),
              rect.height());
      int w = Utils.getWH()[0];
      int h = Utils.getWH()[1];
      zoomedCropBitmap = zoomBitmap(cropBitmap, w, h);
    } catch (Exception e) {
      e.printStackTrace();
    }
    if (cropBitmap != null) {
  ...
  /**
   * 计算InSampleSize 宽的压缩比和高的压缩比的较小值  取接近的2的次幂的值 比如宽的压缩比是3 高的压缩比是5 取较小值3
   * 而InSampleSize必须是2的次幂，取接近的2的次幂4
   */
